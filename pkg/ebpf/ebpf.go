...
func InstallEbpf(linkName string) (*EbpfHandle, error) {
	...
	spec, err := ebpf.LoadCollectionSpecFromReader(bytes.NewReader(VpnLiteProg))
	if err != nil {
	...
	err = netlink.LinkSetXdpFdWithFlags(link, handle.VpnliteProg.FD(), XDP_FLAGS_SKB_MODE)
	if err != nil {
...
type EbpfHandle struct {
	VpnliteProg *ebpf.Program `ebpf:"xdp_vpnlite_prog"`
