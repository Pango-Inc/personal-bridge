...
func dockerImageExists() bool {
	cmd := exec.Command("docker", "image", "inspect", "pbridge-testclient")
	err := cmd.Run()
	return err == nil
}

func dockerImageBuild() {
	slog.Info("Preparing to build docker image pbridge-testclient...")
	tempDir, err := os.MkdirTemp("", "pbridge-image-*")
	if err != nil {
	...
	slog.Info("Building docker image pbridge-testclient...")
	cmd := exec.Command("docker", "build", "-t", "pbridge-testclient", tempDir)
	err = cmd.Run()
	if err != nil {
		slog.Error("Failed to build docker image", slog.Any("err", err))
		os.Exit(1)
	}
	slog.Info("Docker image pbridge-testclient is ready")
}
